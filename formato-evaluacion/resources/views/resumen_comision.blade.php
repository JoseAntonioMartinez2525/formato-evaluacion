            ...
            <table>
                <thead>
                    ...
                    <tr>
                        <th class="evaluadores">
                        @if($userType !== '')   
                            <input class="personaEvaluadora2" type="text" id="personaEvaluadora2">
                        @else
                    ...
                    <tr>
                        <th class="evaluadores">
                        @if($userType !== '')   
                            <input class="personaEvaluadora3" type="text" id="personaEvaluadora3">
                        @else
                        ...
                        try {
                          ...
                          // Generar las filas
                           ...
                           const  minimaTotal = evaluarTotal(totalLogrado);
                            ...
                            //form5
                         // Ahora realiza la solicitud GET para form5 usando el email
                            const evaluatorResponse = await fetch(`/get-evaluator-signature?user_id=${user_id}?email=${email}?user_type=${user_type}`);
                            const evaluatorData = await evaluatorResponse.json();

                            // Manejar los datos de form5 aquí
                            // Por ejemplo, puedes llenar los campos de tu formulario con los datos obtenidos
                            if (evaluatorResponse.ok) {
                                document.getElementById('personaEvaluadora1').value = evaluatorData.evaluator_name_1 || '';
                                document.getElementById('personaEvaluadora2').value = evaluatorData.evaluator_name_2 || '';
                                document.getElementById('personaEvaluadora3').value = evaluatorData.evaluator_name_3 || '';

                                // Si es necesario, puedes mostrar las imágenes de las firmas
                                const imgFirma1 = document.querySelector('img[data-firma="firma1"]');
                                if (imgFirma1) {
                                    imgFirma1.src = evaluatorData.signature_path_1;
                                    imgFirma1.style.display = 'block';
                                }

                                const imgFirma2 = document.querySelector('img[data-firma="firma2"]');
                                if (imgFirma2) {
                                    imgFirma2.src = evaluatorData.signature_path_2;
                                    imgFirma2.style.display = 'block';
                                }

                                const imgFirma3 = document.querySelector('img[data-firma="firma3"]');
                                if (imgFirma3) {
                                    imgFirma3.src = evaluatorData.signature_path_3;
